...
const FEATURED_DISPATCHES: Dispatch[] = [
  ...
  {
    id: "offshore-shell-entities",
    category: "Financial Records",
    title: "Offshore Shell Entities & Beneficial Ownership Registry",
    authorName: "Marcus Chen",
    authorKey: "ed25519:3c7b1a...f402",
    date: "Sept 08, 2026",
    readTime: "11 min read",
    excerpt:
      "Primary source documentation and corporate registration cross-indexes exposing unlisted holding companies in non-cooperative jurisdictions. Cryptographically signed and pinned across three independent networks.",
    cid: "bafybeic52i4f7626vkyz244q56w7g632d4w754z56o2yvshj6c2k7n5wbe",
    href: "/explorer?cid=bafybeic52i4f7626vkyz244q56w7g632d4w754z56o2yvshj6c2k7n5wbe",
  },
  {
    id: "censorship-resistance-democracy",
    category: "Manifesto",
    ...
    readTime: "8 min read",
    excerpt:
      "A foundational treatise on why publishing systems that rely on centralized cloud hosts or domain registries will inevitably yield to coercion when sovereign accountability is challenged.",
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <p className="mt-3 text-base sm:text-lg text-secondary font-light max-w-2xl measure-lead">
              Cryptographically signed investigative articles, unseizable records, and essays preserved across peer-to-peer storage.
            </p>
          </div>
          ...
          >
            <span>Explore network ledger</span>
            <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" />
          </Link>
        </div>

        {/* 3-Column Editorial Grid (Elevation: Card, Inter typography) */}
          ...
          {FEATURED_DISPATCHES.map((dispatch) => (
            ...
            >
              <div>
                ...
                {/* Section 3.1 & 10: Inter ONLY for excerpts */}
                <p className="font-sans text-sm text-secondary leading-relaxed mb-6 line-clamp-4">
                  {dispatch.excerpt}
                </p>
              </div>
              ...
              <div className="pt-4 border-t border-hairline space-y-2.5">
                <div className="flex items-center justify-between text-[11px] font-mono text-secondary">
                  <span className="flex items-center gap-1.5 truncate max-w-[190px]" title={`${dispatch.authorName} (${dispatch.authorKey})`}>
                    ...
                    <span className="text-muted text-[10px]">({dispatch.authorKey.slice(0, 12)}...)</span>
                  </span>
                  <span className="text-muted tabular-nums">{dispatch.date}</span>
                </div>

                {/* CID and Plain Verb (No arrow suffix inside cards) */}
                <div className="flex items-center justify-between text-[10px] font-mono text-muted pt-1">
                  <span className="truncate max-w-[190px]">CID: {dispatch.cid.slice(0, 14)}...</span>
                  <Link
                    href={dispatch.href}
